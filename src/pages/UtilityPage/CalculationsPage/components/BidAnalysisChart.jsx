...
const BidAnalysisChart = ({ bidsData, loading, error }) => {
  ...
  const awardRate = totalBids > 0 ? (awardedBids / totalBids * 100).toFixed(1) : 0;
            ...
            <CardContent>
              ...
              {Object.entries(statusCounts).map(([status, count]) => (
                <Typography key={status} variant="body2">
                  <strong>{status.charAt(0).toUpperCase() + status.slice(1)}:</strong> {count} bids 
                  ({(count / totalBids * 100).toFixed(1)}%)
                </Typography>
              ))}
            </CardContent>
          </Card>
